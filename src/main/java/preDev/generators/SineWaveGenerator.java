...
import static preDev.Test.CHANNEL_NO;
import static preDev.Test.SAMPLE_RATE;
...
public class SineWaveGenerator implements WaveGenerator{
    ...
    @Override
    public short[] generate(double[] freq, int duration, int amplitude, ADSR env) {
        short[] sin = new short[duration * SAMPLE_RATE * CHANNEL_NO];
    ...
    @Override
    public short[] generate(double[] freq, int duration, int amplitude, ADSR env, double modFactor) {
        short[] sin = new short[duration * SAMPLE_RATE * CHANNEL_NO];
        env.setTotalLength(sin.length);
        env.setNoOfTones(freq.length);
        double phase = 0;
    ...
    @Override
    public short[] generate(double[] freq, int duration, int amplitude, ADSR env, double modFactor, ADSR modEnv) {
        short[] sin = new short[duration * SAMPLE_RATE * CHANNEL_NO];
        env.setTotalLength(sin.length);
        env.setNoOfTones(freq.length);
        double phase = 0;
